...
import numpy as np 
import cv2 as cv2
ball   = pd.read_csv("DATA/ball3.csv")
hoop   = pd.read_csv("DATA/hoop3.csv")
poses  = pd.read_csv("DATA/poses3.csv")
...
def plot_shoot(traj, hoop_w=30, hoop_h=30):
...
def is_shot_made(traj, hoop_w=30, hoop_h=30):

    hx_min = HOOP_CX - hoop_w/2
    hx_max = HOOP_CX + hoop_w/2
    hy_min = HOOP_CY - hoop_h/2
    hy_max = HOOP_CY + hoop_h/2
...
if __name__ == "__main__":
    ...
    p0 = poses[poses["person_id"] == 0][["frame"] + head_kps + wrist_kps].copy()
    ...
    for g in groups:
        ...
        if min_dist < 20:
            ...
            if not wrist_in_group.empty:
                ...
                shoot_frame.append(wrist_start)
